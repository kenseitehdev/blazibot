...
[10] To learn moves for your battling experiences, type -moveset and -learn (1-4) (move name) and if there is two words in the move name, put a - where the space is. Right now, pokemon moves are based on their type, so say I had a pikachu, that pikachu would be able to learn every electric move."""
    ...
    async def equip(self, ctx, item:str):
        ...
        if item in itemslist and 'Mega-' not in name :
            ...
            if('mega-stone' in item):
                ...
                evs=evs.replace(",,",",")
                totalevs=evs.split(',')
                if int(totalevs[0])+int(totalevs[1])+int(totalevs[2])+int(totalevs[3])+int(totalevs[4])+int(totalevs[5])>=508:
                    await ctx.send("you have reached your max evs for this pokemon")
                elif (int(totalevs[0])+count)> 245:
                    c.execute(f"update Owned_Pokes set Ev1=252 where Number_Caught={select} and Owner={ctx.author.id}")
                    conn.commit()
                ...
                if int(totalevs[0])+int(totalevs[1])+int(totalevs[2])+int(totalevs[3])+int(totalevs[4])+int(totalevs[5])>=508:
                    ...
                    c.execute(f"update Owned_Pokes set Ev2=252 where Number_Caught={select} and Owner={ctx.author.id}")
                    conn.commit()
                    await ctx.send("you cannot raise this ev any higher")
                else:
                ...
                if int(totalevs[0])+int(totalevs[1])+int(totalevs[2])+int(totalevs[3])+int(totalevs[4])+int(totalevs[5])>=508:
                    ...
                    await ctx.send("you cannot raise this ev any higher")
                    c.execute(f"update Owned_Pokes set ev5=252 where Number_Caught={select} and Owner={ctx.author.id}")
                    conn.commit()
                else:
                ...
                if int(totalevs[0])+int(totalevs[1])+int(totalevs[2])+int(totalevs[3])+int(totalevs[4])+int(totalevs[5])>=508:
                    ...
                    await ctx.send("you cannot raise this ev any higher")
                    c.execute(f"update Owned_Pokes set ev6=252 where Number_Caught={select} and Owner={ctx.author.id}")
                    conn.commit()
                else:
 ...
 | SDf | {i[7]} | Spd | {i[8]} |
  -----------------------
            ...
            t1=type1.split(',')
            if len(t)>1:
                c.execute(f"select * from typeadv where attacking like '{str(t[1])}'")
                type2=c.fetchall()
                type2=str(type2).replace("(","").replace(")","").replace("[","").replace("]","").replace(",,",",")
                t2=type2.split(',')
                more=True
            else:
                more=False
            ...
            superstrong=[]
            while number<19:
                typeb=['Normal','Fire','Water','Electric','Grass','Ice','Fighting','Poison','Ground','Flying','Psychic','Bug','Rock','Ghost','Dragon','Dark','Steel','Fairy']
                if more==True:
                    typa=float(str(t1[number].replace(" ","")))*float(str(t2[number].replace(" ","")))
                else:
                    typa=float(str(t1[number].replace(" ","")))
                if typa==0:
                    immune.append(typeb[number-1])
                elif typa==0.25:
                    superweak.append(typeb[number-1])
                elif typa==0.5:
                    weak.append(typeb[number-1])
                elif typa==1.0:
                    average.append(typeb[number-1])
                elif typa==2.0:
                    strong.append(typeb[number-1])
                elif typa==4.0:
                    superstrong.append(typeb[number-1])
                number=number+1
            sw=""
            w=""
            h=""
            ...
            if str(superstrong)=='[]':
                ...
                for a in superstrong:
                    if ss=="":
                        ss=a
                    else:
                        ss=ss+', '+a
            embed.add_field(name='Immune' , value=str(im), inline=False)
            embed.add_field(name='Super Weak' , value=str(sw), inline=False)
            embed.add_field(name='Weak' , value=str(w), inline=False)
    ...
    async def information(self, ctx, num):
        ...
        base=base.replace(",,",",")
        bases=base.split(',')
        c.execute(f"select hp,atk,def,sp_atk,sp_def,speed,level,Ev1,Ev2,Ev3,Ev4,Ev5,Ev6,Gender,item,Nature,Shiny,EXP,ability,nick,friendship from Owned_Pokes where Number_Caught={pokecaught} and Owner={ctx.author.id}")
        pinfo=c.fetchall()
        pinfo=str(pinfo)
        pinfo=pinfo.replace("(","")
        pinfo=pinfo.replace(")","")
        ...
        total=total*(0.537)
        total=round(total)
        hppower = round((((2*int(bases[0])+int(hpiv)+(int(hpev)/4))*int(level))/100)+int(level)+10)
        atkpower = round((((2*int(bases[1])+int(atkiv)+(int(atkev)/4))*int(level))/100)+5)
        defpower = round((((2*int(bases[2])+int(defiv)+(int(defev)/4))*int(level))/100)+5)
        sp_atkpower = round((((2*int(bases[3])+int(sp_atkiv)+(int(sp_atkev)/4))*int(level))/100)+5)
        sp_defpower = round((((2*int(bases[4])+int(sp_defiv)+(int(sp_defev)/4))*int(level))/100)+5)
        speedpower = round((((2*int(bases[5])+int(speediv)+(int(speedev)/4))*int(level))/100)+5)
        ...
        try:
            ...
            increases=changes[0]
            decreases=changes[1]
            c.execute(f"update Owned_Pokes set Nature='{nature}' where Number_Caught={pokecaught} and Owner={ctx.author.id}")
            conn.commit()
...
|------STATS-------|
  ...
  Total     {total}%"""
                ...
                if egg11==egg21 or egg11==egg22 or egg12==egg21 or egg22==egg22 or '-' in egg11 or '-' in egg12 or '-' in egg21 or '-' in egg22:
                    ...
                    if breedchance==0 and dstat==True:
                        ...
                        if 'Mega' not in name1 and 'Mega' not in name2:
                            ...
                            level2=str(level2)
                            level2=level2[1:-2]
                            if int(level1)>4 and int(level2)>4:
                                st=[]
                                st.append(hp)
                                st.append(atk)
                                st.append(df)
    ...
    @commands.command(name="pp" , aliases=["playing"])
    @charge(amount=10)
    ...
    async def pp(self,ctx):
        ...
        if("None" in exists):
            ...
            await ctx.send(ctx.author.mention+" has been charged 10 BlaziBucks")
            c.execute(f"Select selected from Players where ID='{ctx.author.id}'")
            ...
            c.execute(f"select Name from Pokes where Number={int(selected)}")
            name=c.fetchone()
            name=str(name)
            name=name[2:-3]
            await ctx.send(f"You sent out {name} and played a game!")
            c.execute(f"select friendship from Owned_Pokes where Owner={ctx.author.id} and Number_Caught={numselected}")
            ...
            if int(friendship)< 300:
                ...
                await ctx.send("Your pokemon's friendship is maxed out! youre like best buds! try making friends with another pokemon!")
            if fr%50==0:
                c.execute(f"select level from Owned_Pokes where Owner={ctx.author.id} and Number_Caught={numselected}")
                level=c.fetchone()
                level=str(level)[1:-2]
                lv=int(level)+1
...
[10] To learn moves for your battling experiences, type -moveset and -learn (1-4) (move name) and if there is two words in the move name, put a - where the space is. Right now, pokemon moves are based on their type, so say I had a pikachu, that pikachu would be able to learn every electric move."""
        ...
        if(0==0):
            ...
            c.execute(f"Select friendship from Owned_Pokes where Owner='{message.author.id}' and ID_Owned={numselected}")
            friendship=c.fetchone()
            friendship=str(friendship)[1:-2]
            t=int(totalexp)
            m= int(maxexp)
            if t>m:
                ...
                conn.commit()
                if int(friendship)< 300:
                    c.execute(f"update Owned_Pokes set friendship={int(friendship)+2} where Owner='{message.author.id}' and ID_Owned={numselected};")
                    conn.commit()
                lvl=int(lvl)+1
                if(lvl==5):
